...
1/ 基本脚本
   #1.
      ...
      #!/bin/sh         # 默认shell，linux是bash
   #2. 调用脚本
       sh   scriptname  # 失去一些bash特性
	   bash scriptname
	   chmod 555 scriptname
   #3.echo 显示文本字符串，默认情况下，不需要使用引号将要显示的文本字符串划定出来
          可用单引号或双引号来划定文本字符串

		  -n 使文本字符串 和 命令输出显示在同一行中
     
2/ 命令替换会创建一个子shell来运行对应的命令。      
	     反引号 ``
...
5/ 退出
   shell中运行的每个命令都使用exit status，退出状态码0～255，在命令结束时传给shell(父进程)
...
7/ 测试条件（判断条件）
   ...
   #2.其他测试条件
      有一个专有命令[ ，这个命令与test命令等价, 并且出于效率上的考虑, 这是一个内建命令
	  ...
	  (( ... ))和let 结构也能够返回退出状态码， 它们所测试的算术表达式的结果为非零的时候, 将会返回退出状态码0
...
9/ 用户输入
   ...
   #5. getopt 命令
	   ...
	   根据optstring解析提供的参数，需要参数的选项加冒号
       
	   getopts optstring variable 一次只处理命令行上检测到的一个参数
   #6. 基本读取
       read 命令从标准输入或另一个文件描述符中接受输入
	        接受到输入后，read命令会将数据放进一个变量

	   （1）-p 指定提示符
       （2）输入的每个数据值都会分配给变量列表中的下一个变量
	        变量不足，剩下的数据全部分给最后一个变量
       （3）read命令行中不知定变量，接受到的数据都放进特殊的环境变量REPLY中
	   （4）-t 指定一个计时器，计时器过期，read命令会以非0状态退出
       （5）-s 隐藏模式输入
       （6）每次调用read命令，它都会从文件中读取一行文本。当文件中再没有内容时
	        read命令会退出并返回非0退出状态码
